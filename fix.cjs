...
console.log('OK index.html reecrit proprement');
...
// Supprime le BOM si present
c = c.replace(/^\uFEFF/, '');

// Supprime toute balise <style> contenant @import
c = c.replace(/<style[^>]*>[\s\S]*?@import[\s\S]*?<\/style>/g, '');

// Variante JSX : <style>{`...@import...`}</style>
c = c.replace(/<style>\{`[\s\S]*?@import[\s\S]*?`\}<\/style>/g, '');

fs.writeFileSync(platformPath, c, 'utf8');
console.log('OK Platform.jsx nettoye');

console.log('\nFix termine. Lance maintenant :');
console.log('  git add .');
console.log('  git commit -m "fix index and remove style import"');
